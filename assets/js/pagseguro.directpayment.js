(function(){
    if(window._PagSeguroDirectPayment){return;}
    ...
    var pagsegurosession,PagSeguro=PagSeguro||function(){},gateway=document.createElement("iframe"),styleNode=document.createElement("style"),styleSheets=[".directpaymentgateway {position:absolute; width:0px; height:0px; display:none;}"].join("");
    ...
    PagSeguro.DirectPayment.prototype={
        constructor:PagSeguro.DirectPayment,init:function(){
            ...
            onDocumentReady(function(){
                ...
                if(!window.addEventListener){_that.gateway.attachEvent("onload",_onload);}else{_that.gateway.addEventListener("load",_onload);}
            });
        },
        syntonize:function(){
            this.publish({command:"registerVendor",url:window.location.protocol+"//"+window.location.host},"gateway");
        },
        ...
        setSessionId:function(sessionId){
            ...
            this.queue.push(function(){_that.publish({command:"setSessionId",sessionId:pagsegurosession},"gateway");});
            this.execQueue();
        },
        ...
        execQueue:function(){
            ...
            if(true===this.ready){
                ...
                setTimeout(function(){_that.execQueue();},100);
            }
        },
                ...
                timeout=setTimeout(function(){
                    if(_this.ready&&_this.getSenderHash()){
                        ...
                        if("function"===typeof callback){
                            callback({senderHash:_this.getSenderHash(),status:"success"});
                        }return;
                    }
                    if(!--attempt){
                        return callback({message:"senderHash could not be retrieved",senderHash:null,status:"error"});
                    }
                    checkConditions();
                },1e3);
            };
        ...
        listenChannels:function(){
            ...
            this.subscribe("gateway",function(data){
                if(data.command){
                    switch(data.command){
                        case"createCardToken":_that.runCallback(_that.callbacks.createCardToken,data.result);break;
                        case"getBrand":_that.runCallback(_that.callbacks.getBrand,data.result);break;
                        case"getInstallments":_that.runCallback(_that.callbacks.getInstallments,data.result);break;
                        case"getPaymentMethods":_that.runCallback(_that.callbacks.getPaymentMethods,data.result);break;
                        case"getReady":_that.ready=true;break;
                        case"setDnaHash":_that.setDnaHash(data.result);
                    }
                }
            });
        },
